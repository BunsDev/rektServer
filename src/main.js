...
const saveImage = async (_editionCount) => {
  let path = `${buildDir}/images/${_editionCount}.png`
  fs.writeFileSync(
    `${buildDir}/images/${_editionCount}.png`,
    canvas.toBuffer("image/png")
  );
  let imageUrl = await getLightHouseHash(path)
  console.log("lightHouse Image URL: ", imageUrl)
  return imageUrl
};
...
const saveMetaDataSingleFile = async (_editionCount) => {
  ...
  let path = `${buildDir}/json/${_editionCount}.json`
  fs.writeFileSync(
    `${buildDir}/json/${_editionCount}.json`,
    JSON.stringify(metadata, null, 2)
  );
  let jsonFileUrl = await getLightHouseHash(path)
  console.log("lightHouse json URL: ", jsonFileUrl)
  return jsonFileUrl
};
...
const startCreating = async (userAddress) => {
  let layerConfigIndex = 0;
  let editionCount = 1;
  let failedCount = 0;
  let abstractedIndexes = [];
  let walletObj = await getREKTNft(userAddress)

  console.log("Early data ", layerConfigurations[0])
  getNewLayerConfig(walletObj)
...
const setConfigTodefault = () => {
  ...
  layerConfigurations[0].layersOrder[0].name = "1-bg"
  layerConfigurations[0].layersOrder[1].name = "2-face"
  layerConfigurations[0].layersOrder[2].name = "3-body"
  layerConfigurations[0].layersOrder[3].name = "4-exp"
  layerConfigurations[0].layersOrder[4].name = "5-beard"
  layerConfigurations[0].layersOrder[5].name = "6-acc"
